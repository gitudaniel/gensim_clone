...
def word2vec_similarity(segmented_topics, accumulator, with_std=False, with_support=False):
    ...
    with_support : bool, optional
        ...
        the number of pairwise similarity comparisons that were used to compute the overall topic coherence.
    ...
    if total_oov > 0:
        logger.warning("%d terms are not in word2vec model vocabulary", total_oov)
...
def cosine_similarity(segmented_topics, accumulator, topics, measure='nlr',
    ...
    accumulator : :class:`~gensim.topic_coherence.text_analysis.InvertedIndexAccumulator`
        Output from the probability_estimation module. Is a topics: Topics obtained from the trained topic model.
    ...
    with_support : bool
        ...
        comparisons that were used to compute the overall topic coherence.
...
def _key_for_segment(segment, topic_words):
    """A segment may have a single number or an iterable of them."""
