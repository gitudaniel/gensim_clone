...
def open_file(input):
    ...
    ------
    file
        File-like object based on input (or input if this is already file-like).

    """
    # TODO: ADD 'is' before 'already file-like'
...
def simple_tokenize(text):
    """Tokenize input text using :const:`gensim.utils.PAT_ALPHABETIC`.  # TODO: replace test with text
...
class SaveLoad:
    ...
    def _load_specials(self, fname, mmap, compress, subname):
        ...
        for attrib in getattr(self, '__scipys', []):
            # For insight into `indptr` see https://stackoverflow.com/a/52299730
            logger.info("loading %s from %s with mmap=%s", attrib, subname(fname, attrib), mmap)
    ...
    def _smart_save(
        ...
        separately : list, optional
            Iterable of attributes that need to store distinctly.  # TODO: Replace than with that
        sep_limit : int, optional
            Limit for separation.
        ignore : frozenset, optional
            Attributes that shouldn't be stored.
...
def is_corpus(obj):
    ...
    The peeked element is put back into an object returned by this function, so always use
...
def toptexts(query, texts, index, n=10):
    ...
    index : any
        An instance from :mod:`gensim.similarity.docsim`.  # TODO: remove `from`
...
def randfname(prefix='gensim'):
    ...
    -------
    str
        Full path in the system's temporary folder, ending in a random filename.
...
def effective_n_jobs(n_jobs):
    """Determines the number of jobs that can run in parallel.
