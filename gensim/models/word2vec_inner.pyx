...
cdef unsigned long long w2v_fast_sentence_cbow_neg(
    ...
    cbow_mean
        If 0, use the sum of the context word vectors as the prediction. If 1, use the mean.
    next_random
        Seed for drawing the predicted word for the next iteration of the same routine.
    words_lockf
        Lock factors for each word. A value of 0 will block training.
    _compute_loss
        Whether or not the loss should be computed at this step.
...
def score_sentence_sg(model, sentence, _work):
    ...
    for token in sentence:
        ...
        if word_index is None:
            ...
            # idiosyncratic technique, and only works for the non-default hs model. Here, in lieu of the
            ...
            # single invocation of one of the inner train methods, without changing the model.)
            continue
