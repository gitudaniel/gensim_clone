...
class LdaState(utils.SaveLoad):
    ...
    def load(cls, fname, *args, **kwargs):
        ...
        if not hasattr(result, 'dtype'):
            ...
            logger.info("dtype was not set in saved %s file %s, assuming np.float64", result.__class__.__name__, fname)
...
class LdaModel(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    def show_topic(self, topicid, topn=10):
        """Get the representation for a single topic. Words here are the actual strings, in contrast to
    ...
    def get_topic_terms(self, topicid, topn=10):
        """Get the representation for a single topic. Words the integer IDs, in contrast to
    ...
    def save(self, fname, ignore=('state', 'dispatcher'), separately=None, *args, **kwargs):
        ...
        ignore : tuple of str, optional
            ...
            the internal `state` is ignored by default is that it uses its own serialization rather than the one
    ...
    def load(cls, fname, *args, **kwargs):
        ...
        **kwargs
        ...
        if not hasattr(result, 'random_state'):
            ...
            logger.warning("random_state not set so using default value")
        ...
        try:
            ...
            logger.warning("Failed to load state from %s: %s", state_fname, e)
        ...
        if os.path.isfile(id2word_fname):
            try:
                ...
                logger.warning("Failed to load id2word dicitonary from %s : %s", id2word_fname, e)
        return result
