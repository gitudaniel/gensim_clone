...
class HdpModel(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    **Stick breaking construction**
    ...
    at a new table. Here the probability of choosing each option is not the same.
    ...
    share the same set of atoms, :math:`\Phi_{k}`, and only the atom weights :math:`\pi_{jt}` differs.
    ...
    :math:`\Phi_{k}` with probability :math:`\pi_{jt}`. :math:`C_{j,t}` is an indicator variable whose value `k`
    signifies the index of :math:`\Phi`. This helps to map :math:`\psi_{jt}` to :math:`\Phi_{k}`.

    The top level (`corpus` level) stick proportions correspond to the values of :math:`\beta`,
    ...
    Now, whenever coordinate ascent updates are to be performed, they happen at two levels. The document level as well
    as corpus level.
    ...
    Repeat 2 to 6 until stopping condition is met.
    ...
    * whole corpus processed
    ...
    lda_beta : numpy.ndarray
        Same as :math:`\beta` from :class:`gensim.models.ldamodel.LdaModel`.
    m_D : int
    ...
    m_rhot : float
        Assigns weight to the information obtained from the mini-chunk and its value is between 0 and 1.
    ...
    def doc_e_step(self, ss, Elogsticks_1st, unique_words, doc_word_ids, doc_word_counts, var_converge):
        ...
        doc_word_ids : iterable of int
            Word ids for a single document.
