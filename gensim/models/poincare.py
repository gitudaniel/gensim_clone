...
This module allows training Poincaré Embeddings from a training file containing relations of graphs in a
...
class PoincareModel(utils.SaveLoad):
    ...
    self.kv : :class:`~gensim.models.poincare.PoincareKeyedVectors`
        This object essentially contains the mapping between nodes and embeddings, as well as the vocabulary of the model
    ...
    def build_vocab(self, relations, update=False):
        ...
        update : bool, optional
            If True, only new nodes' embeddings are initialized.
            Use this when the model already has an existing vocabulary and you want to update it.
            If False, all nodes' embeddings are initialized.
...
class PoincareKeyedVectors(KeyedVectors):
    ...
    def similarity(self, w1, w2):
        ...
        -------
        float
            Similarity between the vectors for nodes `w1` and `w2` (between 0 and 1).
        ...
        ------
        KeyError
            If either `w1` and `w2` is absent from vocab.
...
class PoincareRelations:
    ...
    def __init__(self, file_path, encoding='utf8', delimiter='\t'):
        ...
        file_path : str
            ...
            To express a "u is v" relation, the lines should take the form `u delimiter v`.
...
class ReconstructionEvaluation:
    ...
    def get_positive_relation_ranks_and_avg_prec(all_distances, positive_relations):
        ...
        Parameters
        ----------
        all_distances : numpy.array or float
...
class LexicalEntailmentEvaluation:
    ...
    def evaluate_spearman(self, embedding):
        ...
        -------
        float
            Spearman correlation score for input embedding.
