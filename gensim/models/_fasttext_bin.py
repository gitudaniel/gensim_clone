...
def _conv_field_to_bytes(field_value, field_type):
    """
    Auxiliary function that converts `field_value` to bytes based on the request `field_type`,
    ...
    if field_type == 'i':
        ...
        raise NotImplementedError('Currently conversion to "%s" type is not implemented.' % field_type)
...
def _get_field_from_model(model, field):
    ...
    if field == 'bucket':
        ...
        # cbow = continuous bag of words (default)
        # sg = skip-gram
        # sup = supervised
        ...
        msg = 'Extraction of header field "' + field + '" from Gensim FastText object not implemented.'
        raise NotImplementedError(msg)


def _args_save(fout, model, fb_fasttext_parameters):
    """
    Saves header with `model` parameters to the binary stream `fout` containing a model in Facebook's
...
def _dict_save(fout, model, encoding):
    """
    Saves the dictionary from `model` to the binary stream `fout` containing a model in the Facebook's
    native fastText `.bin` format.
    ...
    # prunedidx_size_=-1, -1 value denotes no pruning index (pruning is only supported in supervised mode)
...
def _save_to_stream(model, fout, fb_fasttext_parameters, encoding):
    ...
    fb_fasttext_parameters : dictionary
        dictionary containing  parameters `lr_update_rate`, `word_ngrams`
        unused by gensim implementation, so they have to be provided externally
    encoding : str
...
def save(model, fout, fb_fasttext_parameters, encoding):
    ...
    model : gensim.models.fasttext.FastText
        saved model
    fb_fasttext_parameters : dictionary
        dictionary conaining parameters `lr_update_rate`, 'word_ngrams`
        unused by gensim implementation, so they have to be provided externally
    encoding : str
        encoding used in the output file
