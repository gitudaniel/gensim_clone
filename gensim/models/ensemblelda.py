...
class EnsembleLda(SaveLoad):
    ...
    def add_model(self, target, num_new_models=None):
        ...
        if self.memory_friendly_ttda:
            ...
            # if the user didn't provide a custom number to use
    ...
    def _generate_topic_cluster(self, eps=0.1, min_samples=None):
        ...
        min_samples : int, optional
            defaults to ``int(self.num_models / 2)``, dbscan min neighbors threshold required to consider
...
class CBDBSCAN:
    ...
    1. (A)symmetric distance matrix provided at fit-time (called 'amatrix').
       For the sake of example below, assume there are only five topics (amatrix contains distances with dim 5x5),
       T_1, T_2, T_3, T_4, T_5:
    2. Start by scanning a candidate topic with respect to a parent topic
       (e.g. T_1 with respect to parent None)
    3. Check which topics are nearby the candidate topic using 'self.eps' as a threshold and call them neighbors
       (e.g. assume T_3, T_4, and T_5 are nearby and become neighbors)
    4. If there are more neighbors than 'self.min_samples', the candidate topic becomes a core candidate for a cluster
       (e.g. if 'min_samples=1', then T_1 becomes the first core of a cluster)
    5. If candidate is a core, CheckBack (CB) to find the fraction of neighbors that are either the parent or the
    ...
    6. If candidate is a core, recursively scan the next nearby topic (e.g. scan T_3) labeling the previous topic as
       ...
       4. (e.g. number of neighbors is 1, therefore candidate T_3 becomes a core)
       5. (e.g. CheckBack finds that two of the four parent and parent neighbors are neighbors of candidate T_3.
          Therefore candidate T_3 does NOT get the same label as its parent T_1)
       6. (e.g. Scan candidate T_5 with respect to parent T_3 that has parent_neighbors T_5)
