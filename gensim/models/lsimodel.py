...
class Projection(utils.SaveLoad):
    ...
    and merging them etc. This is done through the higher-level :class:`~gensim.models.lsimodel.LsiModel` class.
    ...
    def merge(self, other, decay=1.0):
        ...
        Warnings
        --------
        The content of `other` is destroyed in the process, so pass this function copy of `other`
        ...
        decay : float, optional
            Weight of existing observations relative to new ones.
        ...
        # The only operation we ever need is basis^T*basis and basis*component.
        ...
        try:
            ...
            # for partial svd/eigendecomp in np :( //sdoering: maybe there's one in scipy?
            ...
            s_k = np.sqrt(s_k)  # go back from eigenvalues to singular values
...
class LsiModel(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    * ``model[training_corpus]`` - right singular vectors (can be reconstructed if needed).
    ...
    def add_documents(self, corpus, chunksize=None, decay=None):
        ...
        decay : float, optional
            Weight of existing observations relative to new ones, will use `self.decay` if not specified.
    ...
    def load(cls, fname, *args, **kwargs):
        ...
        Notes
        -----
        Large arrays can be mmap'ed back as read-only (shared memory) by setting the `mmap='r'` parameter.
