...
class KeyedVectors(utils.SaveLoad):
    ...
    def get_mean_vector(self, keys, weights=None, pre_normalize=True, post_normalize=False, ignore_missing=True):
        ...
        post_normalize : bool, optional
            ...
            If True, normalized mean vector will be returned.
    ...
    def __setitem__(self, keys, weights):
        """Add keys and their vectors in a manual way.
    ...
    def evaluate_word_pairs(
        ...
        """Compute correlation of the model with human similarity judgements.
    ...
    def relative_cosine_similarity(self, wa, wb, topn=10):
        ...
        wb : str
            Word for which we're evaluating relative cosine similarity with wa.
        topn : int, optional
...
def _load_word2vec_format(
    ...
    unicode_errors : str, optional
        default 'strict', is a string suitable to be passed as the `errors`
        argument to the unicode() (Python 2.x) or str() (Python 3.x) function. Your source
        file may include word tokens truncated in the middle of a multibyte unicode character
        (as is common from the original word2vec.c tool), 'ignore' or 'replace' may help.
    limit : int, optional
        Sets a maximum number of word-vectors to read from the file. The default,
