...
def argsort(x, topn=None, reverse=False):
    """Efficiently calculate the indices of the `topn` smallest elements in array `x`.
...
def corpus2csc(corpus, num_terms=None, dtype=np.float64, num_docs=None, num_nnz=None, printprogress=0):
    ...
    num_docs : int, optional
        Number of documents in `corpus`. If provided, the `corpus.num_docs` attribute (if any) will be ignored.
    ...
    try:
        ...
        pass  # not a MmCorpus...
    if printprogress:
        logger.info("creating a sparse matrix from corpus")
...
def full2sparse_clipped(vec, topn, eps=1e-9):
    ...
    This is more efficient than sorting a vector and then taking the greatest values, especially
    where `len(vec) >> topn`.
...
class Sparse2Corpus:
    ...
    def __getitem__(self, key):
        ...
        key: int, ellipsis, slice, iterable object
            Index of the document to retrieve.
...
def qr_destroy(la):
    ...
    Warnings
    --------
    Content of `la` as well as `la[0]` gets destroyed in the process. Again, for memory-efficiency reasons.
