...
def filter_example(elem, text, *args, **kwargs):
    ...
    but you can of course choose not to use some or all parts of the context. Please
...
def remove_template(s):
    ...
    -------
    str
        Copy of `s` with all the `wikimedia markup template <http://meta.wikimedia.org/wiki/Help:Template>`_ removed.

    Notes
    -----
    Since template can be nested, it is difficult to remove them using regular expressions.
...
def remove_file(s):
    ...
    -------
    str
        Copy of `s` with all the 'File:' and 'Image:' markup replaced by their `corresponding captions
...
class WikiCorpus(TextCorpus):
    ...
    * <LANG>wiki-latest-pages-articles.xml.bz2
    ...
    Notes
    -----
    Dumps for the English Wikipedia can be found at https://dumps.wikimedia.org/enwiki/.

    Attributes
    ----------
    metadata : bool
        Whether to write article titles to serialized corpus.
